let quantity2 = parseInt( localStorage.getItem("quantity2") ) || 0
let basket = JSON.parse(localStorage.getItem("cart2")) || []
let number = document.getElementById('number').innerHTML= quantity2
           ...
           `<div class="cart-item">
            <div class ='inside'>
                   ...
                   <div class="buttons">
                           ...
                           <div id="search${basket[i].id}" class="quantity2">${basket[i].quantity}</div>
...
function remove (index){
   quantity2 = quantity2 - basket[index].quantity2
   basket.splice(index,1)
   document.getElementById("number").innerHTML= quantity2
...
let decreaseQuantity=(productNumber)=>{
   basket=JSON.parse(localStorage.getItem("cart2")) || [];
    ...
    if (basket[productNumber].quantity >1){
       basket[productNumber].quantity -= 1
       quantity2-=1
       document.getElementById(`search${basket[productNumber].id}`).innerHTML =  basket[productNumber].quantity2
       document.getElementById("number").innerHTML= quantity2
       localStorage.setItem("cart2",JSON.stringify(basket))
       localStorage.setItem("quantity2",quantity2)
       renderHtml()     
